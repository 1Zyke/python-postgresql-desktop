...
def save_new_student(name, age, address):
    ...
    cursor = conn.cursor()
    query = '''INSERT INTO studentss(name, age, address) VALUES (%s, %s, %s)'''
...
def search(id):
    conn = psycopg2.connect(
        dbname="postgres", 
        user="postgres", 
        password="mipapieslorenzo22", 
        host="localhost", 
        port="5432"
        )
    cursor = conn.cursor()
    query = '''SELECT * FROM studentss WHERE id=%s'''
...
button = Button(frame, text="Add", command=lambda:save_new_student(entry_name.get(), entry_age.get(), entry_adress.get()))
